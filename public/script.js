...
document.getElementById("pay-button").addEventListener("click", async (e) => {
  ...
  <div style="font-family: Arial, sans-serif;
    ...
    <div style="text-align: center; margin-bottom: 20px;">
      <img src="https://firebasestorage.googleapis.com/v0/b/escuelas-235df.appspot.com/o/IJFR%2Fjfr_logo.webp?alt=media&token=69dd35e3-4d3f-4515-84ae-44e994ae2bed" alt="Logo" style="max-height: 80px;">
    </div>
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
      <tr>
        <td style="padding: 8px; color: #555;"><strong>ID de Pago:</strong></td>
        <td style="padding: 8px; text-align: right; color: #333;">${
          paymentIntent.id
        }</td>
      </tr>
      <tr>
        <td style="padding: 8px; color: #555;"><strong>Monto:</strong></td>
        <td style="padding: 8px; text-align: right; color: #333;">${(
          paymentIntent.amount / 100
        ).toFixed(2)} ${paymentIntent.currency.toUpperCase()}</td>
      </tr>
      <tr>
        <td style="padding: 8px; color: #555;"><strong>Método de Pago:</strong></td>
        <td style="padding: 8px; text-align: right; color: #333;">Tarjeta</td>
      </tr>
      <tr>
        <td style="padding: 8px; color: #555;"><strong>Concepto:</strong></td>
        <td style="padding: 8px; text-align: right; color: #333;">Inscripción al Ciclo Escolar 2025-2026</td>
      </tr>
      <tr>
        <td style="padding: 8px; color: #555;"><strong>Fecha y Hora:</strong></td>
        <td style="padding: 8px; text-align: right; color: #333;">${new Date(
          paymentIntent.created * 1000
        ).toLocaleString()}</td>
      </tr>
      <tr>
        <td style="padding: 8px; color: #555;"><strong>Nombre del Pagador:</strong></td>
        <td style="padding: 8px; text-align: right; color: #333;">${name} ${primerApellido} ${segundoApellido}</td>
      </tr>
    </table>
...
`;
    ...
    try {
      const fechaNacimiento = document.getElementById("parent_birthdate").value;
      ...
      const curpFilePadre =
        document.getElementById("parent_curp_file").files[0];
      ...
      const dataPadre = {
        ...
        tipoSangre: document.getElementById("parent_blood_type").value,
        fechaNacimiento: timestamp
          ? firebase.firestore.Timestamp.fromMillis(timestamp)
          : null,
        curp: document.getElementById("parent_curp").value,
...
async function uploadAlumno(idPadre) {
  try {
    const fechaNacimiento = document.getElementById("student_birthdate").value;
    const timestamp = moment(fechaNacimiento, "YYYY-MM-DD").valueOf();
